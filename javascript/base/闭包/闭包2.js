...
//匿名函数能够访问A函数的name和其他变量，这就叫做闭包。即使这个内部函数被返回了，而且是在其他地方被调用了，但它仍然可以访问变量propertyName。
...
//compare初始化时，它的作用域链被初始化为包含A函数的活动对象和全局变量对象（关系图见闭包作用域关系图.png）。这样，匿名函数就可以访问在A中定义的所有变量。
